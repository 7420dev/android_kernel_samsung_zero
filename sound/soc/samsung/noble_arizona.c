...
static int noble_get_dsm_cal_info(struct snd_soc_card *card)
{
	struct arizona_machine_priv *priv = card->drvdata;
	int ret;
...
static int noble_set_dsm_cal_info(struct snd_soc_card *card)
{
	struct arizona_machine_priv *priv = card->drvdata;
	int ret = 0;
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		if (priv->dsm_cal_read_cnt == 0)
			noble_get_dsm_cal_info(card);
		if (priv->dsm_cal_read_cnt == 1)
			noble_set_dsm_cal_info(card);
#endif
		break;
